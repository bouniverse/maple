...
// ── 헬퍼: 일반몹 스테이지 방어율 (2026-06-08 16존 전구간 측정) ──
// g = 글로벌스텝 = (존-1)×10 + 스텝.  방어0 기준 = 2-1(g11).
// 측정: 정밀세팅 논크리/크리, 다이스 발동행 제외 34행. 방어=최종데미지 곱셈 감쇄(크리/논크리 동일).
//   선형  : 0.1915g - 0.385          (R²=0.99501) — 16존 내(g11~165) 보간용
//   2차   : -2.591e-4 g² + 0.2358g - 1.691 (R²=0.99812) — 후반 둔화 반영, 외삽도 이쪽
// 반환: 감쇄율(0~1). 실데미지 × (1 - rate).
export function normalMobDefenseRate(g, { model = 'quad' } = {}) {
  const pct = model === 'linear'
    ? 0.1915 * g - 0.385
    : -2.591e-4 * g * g + 0.2358 * g - 1.691;
...
export function realDamage(s, mode, options = {}) {
  ...
  // 일반몹 스테이지 방어 감쇄 (옵션): 일반몹 + mode.stageG 제공 시만 적용.
  // 보스는 별도 방어경로(미적합)라 여기 미반영. stageG 없으면 1 → 기존 동작 유지.
  const defMul   = (!mode.boss && mode.stageG != null)
    ? (1 - normalMobDefenseRate(mode.stageG, { model: mode.defModel }))
    : 1;
